...
public class SwiftCodeValidator implements Validator {
    ...
    private void isLocationCodeValid(String locationCode, Errors errors) {
        if (!locationCode.matches("^[A-Z0-9]{2}$")) {
            errors.reject("swift-code.location-code.invalid", "Location code must contain 2 alphanumeric characters");
        }
    }

    private void isBranchCodeValid(String branchCode, Errors errors) {
        if (!branchCode.matches("^(XXX|[A-Z0-9]{3})$")) {
            errors.reject("swift-code.branch-code.invalid", "Branch code must be 'XXX' for head office or 3 alphanumeric characters");
        }
    }
}
